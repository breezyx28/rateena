...
const Application = () => {
  document.title = "Application | Rateena - E-Shop Admin Panel";
  ...
  const filterData = () => {
    ...
    const filteredData = appList.filter((item: any) => {
      ...
      const dateMatch =
        date.length === 0 ||
        (new Date(item.date) >= new Date(date[0]) &&
          new Date(item.date) <= new Date(date[1]));
        ...
        cell: (cell: any) => (
          <>
            <div className="d-flex align-items-center">
              ...
              <div className="flex-grow-1 ms-2 ">{cell.getValue()}</div>
            </div>
          </>
        ),
      },
